...
default:	$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.wikidot \
		$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p-histogram.pdf
...
$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.txt: $(OUTPUT_DIR)/new-hum-disease-validation-tuples.txt $(PRED_DIR)/$(PROFILE_GD_PREFIX)/hum-disease-$(REF_SOURCE)-profiles.txt
	python filter_file.py $(OUTPUT_DIR)/new-hum-disease-validation-tuples.txt $(PRED_DIR)/$(PROFILE_GD_PREFIX)/hum-disease-$(REF_SOURCE)-profiles.txt 2 > $@.tmp
	mv $@.tmp $@

$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.wikidot: \
		$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.txt \
		wikiformat-results.sh
	sort -n -t "|" -k 12,2 $(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.txt | head -n 100 > $@.tmp
	export REF_SOURCE=$(REF_SOURCE) && cat $@.tmp | sh wikiformat-results.sh > $@.tmp2
	rm $@.tmp ; mv $@.tmp2 $@

$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p-histogram.txt: \
		$(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.txt
	cat $(OUTPUT_DIR)/new-hum-disease-validation-tuples-pred-$(REF_SOURCE)-p.txt |  python histogram.py 2 > $@.tmp
	mv $@.tmp $@
